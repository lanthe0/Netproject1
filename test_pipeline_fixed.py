...
def run_test_suite():
    ...
    for test_id, input_file, content_type in test_files:
        print(f"\n{'=' * 60}")
        print(f"[TEST] {test_id} ({content_type})")
        print(f"{'=' * 60}")
        
        file_size = os.path.getsize(input_file)
        print(f"[INFO] 输入文件大小: {file_size:,} bytes ({file_size/1024:.1f} KB)")
    ...
    # 详细结果表格
    print("\n" + "-" * 90)
    print(f"{'测试ID':<20} {'大小':<10} {'编码':<8} {'解码':<8} {'匹配':<6} {'编码时间':<10} {'解码时间':<10}")
    print("-" * 90)
    
    for r in results:
        size_kb = r['file_size'] / 1024
        encode_mark = 'OK' if r['encode_status'] == 'success' else 'FAIL'
        decode_mark = 'OK' if r['decode_status'] == 'success' else 'FAIL'
        match_mark = 'OK' if r['data_match'] else 'FAIL'
        encode_time_str = f"{r['encode_time']:.2f}s" if r['encode_time'] > 0 else "-"
        decode_time_str = f"{r['decode_time']:.2f}s" if r['decode_time'] > 0 else "-"
        
        print(f"{r['id']:<20} {size_kb:>8.1f}KB {encode_mark:<8} {decode_mark:<8} {match_mark:<6} {encode_time_str:<10} {decode_time_str:<10}")
